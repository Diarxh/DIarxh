...
@section('content')
    ...
    <script>
        function showRegistrationConfirm(trainingId, trainingName, trainingPhoto, trainingLocation) {
            const trainingPhotoUrl = trainingPhoto ?
                `{{ asset('storage') }}/${trainingPhoto}` :
                `{{ asset('theme/img/blog-1.png') }}`;

            Swal.fire({
                title: 'Konfirmasi Pendaftaran Pelatihan',
                html: `
            <div class="text-center">
                <img src="${trainingPhotoUrl}" alt="${trainingName}" style="width: 100px; height: auto; margin-bottom: 15px;"/>
                <p><strong>Nama Pelatihan:</strong> ${trainingName}</p>
                <p><strong>Lokasi:</strong> ${trainingLocation}</p>
